...
On the `/beers` route, we should display all the beers from the Database. So, in this case, you need to call the API `https://ironbeerapi.herokuapp.com/beers/all`. The API will return an array of `Beers` objects.
...
On the `/random-beer` route, we will render a single beer that will be retrieved from the database. The endpoint will do all the job for us, so we need to call to `https://ironbeerapi.herokuapp.com/beers/random`. We should receive an object including all the info about a beer. The same way we did with the **Single Beer** view, we should render the following fields:
...
Then, you should do a `POST` request to `https://ironbeerapi.herokuapp.com/beers/new`, passing all the data on the `body` object. If everything went ok, you would receive a **200** response from the server.
